...
CREATE TABLE IF NOT EXISTS `interpol`.`record` (
...
CREATE TABLE IF NOT EXISTS `interpol`.`wanted_person` (
  ...
  CREATE TABLE IF NOT EXISTS `interpol`.`request` (
  `id` INT NULL AUTO_INCREMENT,
  `reward` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `interpol`.`record_wanted_person` (
  `id` INT NULL AUTO_INCREMENT,
  `wanted_person_id` INT NOT NULL,
  ...
  CONSTRAINT `fk_record_wanted_person_record1`
    ...
    REFERENCES `interpol`.`record` (`id`)
    ON DELETE CASCADE);

CREATE TABLE IF NOT EXISTS `interpol`.`user_request` (
  `id` INT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_user_request_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `interpol`.`user` (`id`)
    ON DELETE CASCADE,
  CONSTRAINT `fk_user_request_request1`
    FOREIGN KEY (`request_id`)
    REFERENCES `interpol`.`request` (`id`)
    ON DELETE CASCADE);